(function () {
    ...
    class MinasonaTwitchExtension extends FrankerFaceZ.utilities.addon.Addon {
      ...
      onEnable() {
        window.addEventListener('message', ((event) => {
          ...
          const index: number = event.data.FFZ_MINASONATWITCHEXTENSION_BADGE.index;

          if (!this.communities.includes(community))// register community if missing
            this.registerTemplate(community);

          this.registerUserBadge(index, community, userId, username, imageUrl, iconUrl, isGeneric);
        }).bind(this));
       ...
       */
      async registerUserBadge(index: number, community: string, userId: string, username: string, imageUrl: string, iconUrl: string, isGeneric: boolean) {
  ...
  function ffzScriptObserver_callback(mutationsList: MutationRecord[], observer: MutationObserver) {
    ...
    window.postMessage({ FFZ_MINASONATWITCHEXTENSION_SETUP: true });
